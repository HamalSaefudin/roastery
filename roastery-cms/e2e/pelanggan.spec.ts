...
test('sidebar Pelanggan punya sub-menu dan halaman daftar termuat', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'Pelanggan' }).first().click()
  await expect(page.getByRole('heading', { name: 'Pelanggan' })).toBeVisible()
  expect(errors, `Console error:\n${errors.join('\n')}`).toHaveLength(0)
})

test('halaman daftar pelanggan termuat dengan tabel dan search', async ({ page }) => {
  await buka(page, '/login')
  await page.getByLabel('Email').fill(email)
  await page.getByLabel('Password').fill(password)
  await page.getByRole('button', { name: 'Masuk' }).click()

  await page.getByRole('link', { name: 'Daftar Pelanggan' }).click()
  await expect(page.getByRole('heading', { name: 'Pelanggan' })).toBeVisible()
  await expect(page.getByRole('table')).toBeVisible()
  await expect(page.getByPlaceholder('Cari nama/email/kode...')).toBeVisible()
})

test('halaman Pengajuan Wholesale termuat dengan tabel dan filter', async ({ page }) => {
  const errors = kumpulkanErrorConsole(page)
  await buka(page, '/login')
  await page.getByLabel('Email').fill(email)
  await page.getByLabel('Password').fill(password)
  ...
  await page.getByRole('link', { name: 'Pengajuan Wholesale' }).click()
  await expect(page.getByRole('heading', { name: 'Pengajuan Wholesale' })).toBeVisible()
  await expect(page.getByRole('table')).toBeVisible()
  expect(errors, `Console error:\n${errors.join('\n')}`).toHaveLength(0)
})
