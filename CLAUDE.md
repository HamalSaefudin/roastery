...
| 07 | Pelanggan & Wholesale | ✅ selesai — sidebar sub-menu (Daftar/Pengajuan Wholesale), list customer (search + filter tipe + pagination + kode mono + StatusBadge tipe), detail customer (profil + ringkasan), pengajuan wholesale (filter status + approve ConfirmDialog nama usaha + reject dialog alasan wajib textarea min 10 char) — typecheck/lint/check/build hijau |
